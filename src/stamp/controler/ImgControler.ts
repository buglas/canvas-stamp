...
/* 变换之前的暂存数据类型 */
type TransformStage = {
	imgWorldCenter: Vector2
	worldOpposite: Vector2
	imgWorldMatrixInvert: Matrix3
}
...
class ImgControler extends Object2D {
	...
	transformStage: TransformStage = {
		imgWorldCenter: new Vector2(),
		worldOpposite: new Vector2(),
		imgWorldMatrixInvert: new Matrix3(),
	}
	// 图像变换器
	...
	set img(val) {
		...
		this._img = val
		if (val) {
			const { imgTransformer, frame, controlStage } = this
			imgTransformer.setOption({ img: val })
	...
	set controlState(val) {
		...
		const { img, imgTransformer } = this
		if (!val || !img) {
			return
		}
		const { localOrign } = this
		this.setTransformStage(img)
		...
		if (val !== 'move') {
			...
			// 在不改变图像世界位的前提下，设置图像基点
			this.updateImgOrigin(localOrign, img)
		}
	}
	...
	setRotateOrigin() {
		const {
			localOrign,
			imgTransformer,
			transformStage: { imgWorldCenter, imgWorldMatrixInvert },
		} = this
		localOrign.copy(imgWorldCenter.clone().applyMatrix3(imgWorldMatrixInvert))
	...
	setScaleOrigin() {
		const {
			...
			localOrign,
			imgTransformer,
			transformStage: { imgWorldCenter, worldOpposite, imgWorldMatrixInvert },
		} = this
		localOrign.copy(
			(altKey ? imgWorldCenter : worldOpposite)
				.clone()
				.applyMatrix3(imgWorldMatrixInvert)
		)
		imgTransformer.updateOriginToMouseStart()
	}

	/* 变换之前，暂存图像和鼠标数据 */
	setTransformStage(img: Img) {
		const {
			clipMousePos,
			imgTransformer,
			frame,
		...
		imgWorldMatrixInvert.copy(img.clipWorldMatrix.invert())
		imgWorldCenter.copy(frame.center)
		worldOpposite.copy(frame.opposite)
		imgTransformer.setOption({
			img,
			mouseStart: clipMousePos.clone().applyMatrix3(imgWorldMatrixInvert),
		})
	}
	...
	pointermove(mp: Vector2) {
		...
		const { clipMousePos, frame } = this

		// 更新鼠标世界位
		clipMousePos.copy(mp)

		// 获取鼠标状态
		this.mouseState = frame.getTransformState(mp)
		...
		if (img) {
			switch (key) {
				case 'Enter':
					...
					break
				case 'Escape':
					imgTransformer.setImgData(controlStage)
					this.img = null
					this.dispatchEvent(_changeEvent)
					break
				case 'Delete':
					this.dispatchEvent({ type: 'remove', img })
					img.remove()
					this.img = null
					this.dispatchEvent(_changeEvent)
					break
	...
	updateLocalMousePos() {
		const {
			clipMousePos,
			localMousePos,
			transformStage: { imgWorldMatrixInvert },
		} = this
		localMousePos.copy(clipMousePos.clone().applyMatrix3(imgWorldMatrixInvert))
	}
	...
	/* 基于本地偏移坐标系设置基点*/
	updateImgOrigin(localOrign: Vector2, img: Img) {
		...
		// 图像的offset需要基于curOffset 做反向偏移
		offset.copy(curOffset)
		// 上一级的position 再偏移回来，以确保图像的世界位不变
